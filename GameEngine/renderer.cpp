...
void Renderer::draw(const glm::vec3 & position, const glm::vec2 & size, const Texture & texture, const glm::vec2 & texturePosition, const glm::vec2 & textureSize)
{
	...
	_shader->setUniform3f("tSize", glm::vec3(tSize, 0.0f));
...
void TextRenderer::drawText(const std::string& text, const Font * font, glm::vec3 position, const glm::vec4& color)
{
	...
	for(char c : text)
	{
		...
		glm::mat4 model(1.0f);
		model = glm::translate(model, glm::vec3(position.x + character.bearing.x, position.y - (character.size.y - character.bearing.y), 0.0f));
